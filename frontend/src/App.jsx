  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px" }}>
        ...
        <select
          style={selectStyle}
          value={String(filters.minPop)}
          onChange={(e) =>
            onFilter({ ...filters, minPop: Number(e.target.value) })
          }
        >
          {[
            ["0", "Any"],
...
export default function App() {
  ...
  const fetchAQI = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        setLastUpdate(data[0].measured_at || null);
      }
    } catch (e) {
  ...
  const ICON_BTN = ({ onClick, title, active, children }) => (
    <button
      ...
      onMouseLeave={(e) => {
        e.currentTarget.style.color = active ? c.accent : c.textMuted;
      }}
    >
      {children}
    </button>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {activeTab === "uv" && (
          <UVIndex data={aqiData} loading={loading} theme={theme} />
        )}
