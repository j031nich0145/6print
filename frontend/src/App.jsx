...
const KPI_DEFS = [
  { id: "top5worst",      label: "5 Worst AQI Cities",    defaultEdge: "top-right"  },
  { id: "top5best",       label: "5 Best AQI Cities",     defaultEdge: "top-right"  },
...
const COLORMAPS = [
  { id: "aqi",     label: "AQI Standard" },
  { id: "viridis", label: "Viridis"      },
  { id: "plasma",  label: "Plasma"       },
  { id: "earth",   label: "Earth"        },
  { id: "rdylgn",  label: "Red → Green"  },
];
...
function CompactThemePicker() {
  // Call useTheme directly — works regardless of what setter name ThemeProvider uses
  const ctx  = useTheme();
  const theme = ctx.theme;
  const c    = theme.colors;
  const mono = theme.typography.fontFamilyMono;

  // ThemeProvider exposes switchTheme(themeObject)
  const applyTheme = ctx.switchTheme ?? ctx.setTheme ?? ctx.setActiveTheme ?? (() => {});

  return (
    <div>
      <div style={{ fontFamily: mono, fontSize: 9, letterSpacing: "0.12em",
        color: c.textSubtle, textTransform: "uppercase", marginBottom: 10 }}>
        Active: <span style={{ color: c.accent }}>{theme.meta.name}</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
        {BUILT_IN_THEMES.map((t) => {
          const active = theme.meta.id === t.meta.id;
          return (
            <button key={t.meta.id} onClick={() => applyTheme(t.meta.id)} style={{
...
// ── Settings Modal ────────────────────────────────────────────────────────────
function SettingsModal({ open, onClose, theme, onThemeSelect,
  kpiState, onKpiToggle, onKpiTemplateSave, onKpiTemplateLoad,
  colormap, onColormap }) {
  const c = theme.colors, mono = theme.typography.fontFamilyMono;
  const templates = JSON.parse(localStorage.getItem("kpi_templates") || "{}");
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width:"min(580px, 95vw)", maxHeight:"80vh",
        ...
        <div style={{ padding:"16px 20px", borderBottom:`1px solid ${c.border}`,
          ...
          <div style={{ fontFamily:mono, fontSize:11, letterSpacing:"0.18em",
            textTransform:"uppercase", color:c.text, fontWeight:700 }}>
            ⚙ Settings
          </div>
        ...
        <div style={{ overflowY:"auto", flex:1 }}>
          ...
          {/* KPI Cards */}
          <Accordion title="KPI Cards" theme={theme} defaultOpen>
              ...
              {KPI_DEFS.map(({ id, label }) => {
                ...
                return (
                  <label key={id} style={{
                    ...
                    <span style={{ fontFamily:mono, fontSize:11, color:c.text }}>{label}</span>
                    {active && (
            ...
            <div style={{ display:"flex", gap:8, marginTop:4 }}>
              <button onClick={onKpiTemplateSave} style={{
                flex:1, padding:"7px", background:c.accentSubtle,
                border:`1px solid ${c.accent}`, borderRadius:theme.shape.buttonRadius,
                color:c.accent, fontFamily:mono, fontSize:10, cursor:"pointer",
              }}>Save Layout</button>
              <button onClick={onKpiTemplateLoad} style={{
                flex:1, padding:"7px", background:c.surface,
                border:`1px solid ${c.border}`, borderRadius:theme.shape.buttonRadius,
            ...
            <div style={{ fontFamily:mono, fontSize:9, color:c.textSubtle,
              ...
              Drag cards to reposition · snap to edges automatically
            </div>
          </Accordion>
          ...
          {/* Plots & Colormaps */}
          <Accordion title="Plots & Colormaps" theme={theme}>
            <div style={{ fontFamily:mono, fontSize:9, color:c.textSubtle,
              marginBottom:10, letterSpacing:"0.08em" }}>
              Color scale applied to choropleth and metric dots
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {COLORMAPS.map((cm) => {
                const active = colormap === cm.id;
                return (
                  <button key={cm.id} onClick={() => onColormap(cm.id)} style={{
                    ...
                    <span style={{ fontFamily:mono, fontSize:10,
                      color: active ? c.accent : c.textMuted, fontWeight: active ? 600 : 400 }}>
                      {cm.label}
                    </span>
                  </button>
                );
          ...
          {/* About */}
          <Accordion title="About" theme={theme}>
            <div style={{ fontFamily:mono, fontSize:10, color:c.textMuted, lineHeight:1.9 }}>
              ...
              <div>Open-Meteo API → AWS Lambda (Python 3.12)</div>
              <div>→ S3 Data Lake → Snowflake Analytics</div>
              <div>→ FastAPI → React</div>
              <div style={{ color:c.textSubtle, fontSize:9, letterSpacing:"0.14em",
                textTransform:"uppercase", margin:"12px 0 6px" }}>Details</div>
              <div>157 cities · 30 min refresh · 2023–present</div>
              <div>Historical: 178,687 rows · 2023-01-01 to now</div>
              <div style={{ color:c.textSubtle, fontSize:9, letterSpacing:"0.14em",
                textTransform:"uppercase", margin:"12px 0 6px" }}>Source</div>
...
function FilterPanel({ open, filters, onFilter, onRefresh, loading, cityCount,
  ...
  const sec = (t) => (
    <div style={{ fontFamily:mono, fontSize:9, letterSpacing:"0.2em",
      ...
      borderBottom:`1px solid ${c.border}`, paddingBottom:6, marginBottom:14, marginTop:4 }}>{t}</div>
  );
      ...
      <div style={{ flex:1, overflowY:"auto", padding:14 }}>
        ...
        {viewMode === "map" && (
          <>
            ...
            <div style={{ fontFamily:mono, fontSize:9, color:c.textSubtle, marginTop:8, lineHeight:1.5 }}>
              {choropleth === "country"
                ? "Mean metric per country fill" : "City dots by selected metric"}
            </div>
          </>
        )}
        <div style={{ marginTop:16 }}>
          <button onClick={onRefresh} disabled={loading} style={{
            width:"100%", padding:8, background:c.accentSubtle,
            border:`1px solid ${c.accent}`, borderRadius:theme.shape.buttonRadius,
            color:c.accent, fontFamily:mono, fontSize:11, letterSpacing:"0.1em",
            cursor:loading?"wait":"pointer", opacity:loading?0.6:1,
          }}>{loading ? "Loading..." : "↺  Refresh"}</button>
        </div>
        <div style={{ fontFamily:mono, fontSize:9, color:c.textSubtle,
          marginTop:14, lineHeight:1.8, letterSpacing:"0.06em" }}>
...
// ── KPI cards layer (rendered above everything) ───────────────────────────────
function KpiCardsLayer({ kpiState, data, filters, metricMeta, theme, onPositionSave }) {
  const visibleCards = kpiState.filter((k) => k.visible);

  // Group visible cards by their default edge, for stacking
  const edgeIndex = {};
  visibleCards.forEach((k) => {
    const def = KPI_DEFS.find((d) => d.id === k.id);
    const edge = def?.defaultEdge ?? "top-right";
    if (edgeIndex[edge] === undefined) edgeIndex[edge] = 0;
  });
  const edgeCounts = {};

  const getPos = (id) => {
    const k = kpiState.find((c) => c.id === id);
    // User has manually positioned this card — use saved position
    if (k?.x != null && k?.y != null) return { x: k.x, y: k.y };

    // Calculate default stacked position
    ...
    const PAD = 12, CARD_W = 210, CARD_H = 200, STACK = 195;
    const x = edge.includes("right") ? W - CARD_W - PAD : PAD;
    const y = edge.startsWith("top")
      ? TAB_H + PAD + idx * STACK
      : H - CARD_H - PAD - idx * STACK;
    return { x, y };
  };

  const isVisible = (id) => kpiState.find((k) => k.id === id)?.visible;
...
export default function App() {
  ...
  const handlePositionSave = (id, x, y) => {
    if (x === -9999) {
      // Dismiss signal from card × button — toggle off
  ...
  const handleTemplateLoad = () => {
    ...
    if (!keys.length) { alert("No saved templates."); return; }
    const name = prompt(`Load template:\n${keys.join(", ")}`);
    if (name && templates[name]) setKpiState(templates[name]);
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh",
      ...
      <div style={{ height:TAB_H, display:"flex", alignItems:"stretch",
        ...
        <ICON_BTN onClick={() => setFiltersOpen((v) => !v)}
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center" }}>
          ...
          <ICON_BTN onClick={() => setSettingsOpen((v) => !v)}
            title="Settings" active={settingsOpen}>⚙</ICON_BTN>
        </div>
      </div>
      ...
      {/* Settings modal (centered, not sidebar) */}
      <SettingsModal open={settingsOpen} onClose={() => setSettingsOpen(false)}
        theme={theme} onThemeSelect={(t) => { setTheme(t); }}
        kpiState={kpiState} onKpiToggle={handleKpiToggle}
        onKpiTemplateSave={handleTemplateSave}
        onKpiTemplateLoad={handleTemplateLoad}
        colormap={colormap} onColormap={setColormap} />

      {/* KPI cards — float above everything */}
